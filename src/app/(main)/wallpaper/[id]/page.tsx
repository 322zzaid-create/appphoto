...
import { useWallpaper } from "@/lib/hooks/useWallpapers";
...
function WallpaperDetailContent({ id }: { id: string }) {
  const router = useRouter();
  const { user, isLoading: authLoading } = useAuth();
  const { toggleFavorite, isFavorited } = useFavorites();
  const { toggleLike, isLiked } = useLikes();
  const { data: wallpaper, isLoading, refetch } = useWallpaper(id);  const { userRating, rate, isPending: isRatingPending } = useRatings(
    id,
