...
describe("requestDownloadToken", () => {
  it("requires a nonce", async () => {
    ...
    const res = await requestDownloadToken({ admin, auth }, { wallpaperId: "w1", nonce: "", ip: null });
    expect(res.ok).toBe(false);
    if (!res.ok) expect(res.status).toBe(400);
  ...
  it("rejects an unknown wallpaper", async () => {
    const { state, admin, auth } = makeDb({
      rows: { ...makeDb().state.rows, ad_confirmations: [{ id: "c1" }] },
    });
    const res = await requestDownloadToken({ admin, auth }, { wallpaperId: "nope", nonce: "n1", ip: null });
    expect(res.ok).toBe(false);
    if (!res.ok) expect(res.status).toBe(404);
    expect(state.updated).toHaveLength(0);
  });

  it("rejects when the ad was never confirmed", async () => {
    const { state, admin, auth } = makeDb({
      rows: { ...makeDb().state.rows, wallpapers: [publishedWallpaper()] },
    });
    const res = await requestDownloadToken({ admin, auth }, { wallpaperId: "w1", nonce: "n1", ip: "1.2.3.4" });
    expect(res.ok).toBe(false);
    if (!res.ok) expect(res.status).toBe(403);
    expect(state.updated).toHaveLength(0);
  });

  it("issues a usable token and consumes the confirmation", async () => {
    const { state, admin, auth } = makeDb({
      rows: {
        ...makeDb().state.rows,
        wallpapers: [publishedWallpaper()],
        ad_confirmations: [{ id: "c1", nonce: "n1", wallpaper_id: "w1" }],
      },
    });
    const res = await requestDownloadToken({ admin, auth }, { wallpaperId: "w1", nonce: "n1", ip: "1.2.3.4" });
    expect(res.ok).toBe(true);
    ...
    expect(state.updated[0].table).toBe("ad_confirmations");
  });

  it("does not consume the confirmation when rate-limited", async () => {
    const { state, admin, auth } = makeDb({
      counts: { downloads: 5 },
      rows: {
        ...makeDb().state.rows,
  ...
  it("rejects premium wallpapers for guests", async () => {
    const { state, admin, auth } = makeDb({
      rows: {
        ...
        ad_confirmations: [{ id: "c1" }],
      },
    });
    const res = await requestDownloadToken({ admin, auth }, { wallpaperId: "w1", nonce: "n1", ip: null });
    expect(res.ok).toBe(false);
    if (!res.ok) expect(res.status).toBe(403);
    expect(state.updated).toHaveLength(0);
  });
